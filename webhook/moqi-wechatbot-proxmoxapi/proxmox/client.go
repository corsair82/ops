...
type NodeSource struct {
	CanUseCpu      int
	CanUseMem      int
	CanUseGraphics []string
}

func NewNodeSource(cpu, mem int, graphics []string) NodeSource {
	return NodeSource{
		CanUseCpu:      cpu,
		CanUseMem:      mem,
		CanUseGraphics: graphics,
	}
}
func NoIntersetion(a, b []string) []string {
	var longslice, shortslice, c []string
	if len(a) > len(b) {
		longslice, shortslice = a, b
	} else {
		longslice, shortslice = b, a
	}
	tmap := make(map[string]int)
	for _, v := range shortslice {
		tmap[v] = 1
	}
	for _, v := range longslice {
		_, isexsit := tmap[v]
		if !isexsit {
			c = append(c, v)
		}
...
func (c *Client) GetNodeRemainSource() map[string]NodeSource {

	nodesinfo, err := c.GetNodeList()
	if err != nil {
		log.Printf("Error listing Node %+v\n", err)
		os.Exit(1)
	}
	nodeinfo, isnode := nodesinfo["data"].([]interface{})
	if isnode {
		var (
			nodename string
			// nodecanusegraphics int
			nodecpu, nodemem, usedcpu, usedmem           int
			usedgraphics, nodegraphics, nodegraphicsinfo []string
		)
		nodessource := make(map[string]NodeSource)
		for node := range nodeinfo {
			nodegraphicsinfo = []string{}
			sourceinfo := nodeinfo[node].(map[string]interface{})
			nodename = string(sourceinfo["node"].(string))
			nodepics, nodepicsinfo := c.GetNodePcis(nodename)
			usedcpu, usedmem, usedgraphics = c.GetNodeVmSourceUsed(nodename)
			nodecpu = int(math.Ceil(sourceinfo["maxcpu"].(float64))) - usedcpu
			nodemem = int(math.Ceil(sourceinfo["maxmem"].(float64))) - usedmem
			nodegraphics = NoIntersetion(nodepics, usedgraphics)

			for _, v := range nodegraphics {
				if picdinfo, ok := nodepicsinfo[v]; ok {
					piciddinfo := v + ":" + pciname[picdinfo]
					nodegraphicsinfo = append(nodegraphicsinfo, piciddinfo)
				}
			}
			nodessource[nodename] = NewNodeSource(nodecpu, nodemem, nodegraphicsinfo)
		}
...
func (c *Client) GetNodeVmList(node string) (nodePcis map[string]interface{}, err error) {
	url := fmt.Sprintf("/nodes/%s/qemu", node)
	err = c.GetJsonRetryable(url, &nodePcis, 3)
	return

}
func (c *Client) GetNodeVmSourceUsed(node string) (usedcpu, usedmem int, usedpcis []string) {

	vms, err := c.GetNodeVmList(node)
	if err != nil {
		log.Printf("Error listing NodePcis %+v\n", err)
		os.Exit(1)
	}
	vmsinfo, isvm := vms["data"].([]interface{})
	if isvm {
		for vm := range vmsinfo {
			vminfo := vmsinfo[vm].(map[string]interface{})
			if string(vminfo["status"].(string)) == "running" {
				usedcpu = usedcpu + int(math.Ceil(vminfo["cpus"].(float64)))
				usedmem = usedmem + int(math.Ceil(vminfo["maxmem"].(float64)))
				vmid, _ := strconv.Atoi(string(vminfo["vmid"].(string)))
				vmr := &VmRef{vmId: vmid, node: node, vmType: "qemu"}
				vmconfig, _ := c.GetVmConfig(vmr)
				for k := range vmconfig {
					if strings.Contains(k, "hostpci") {
						pciinfo := strings.Split(string(vmconfig[k].(string)), ",")
						pciid := "0000:" + pciinfo[0] + ".0"
						usedpcis = append(usedpcis, pciid)
					}
				}
			}
		}
		return usedcpu, usedmem, usedpcis
	}
	return 0, 0, nil
}

func (c *Client) GetNodePciList(node string) (nodePcis map[string]interface{}, err error) {
	url := fmt.Sprintf("/nodes/%s/hardware/pci", node)
	err = c.GetJsonRetryable(url, &nodePcis, 3)
	return

}
func (c *Client) GetNodePcis(node string) (pcislice []string, pciiddeviceinfo map[string]string) {
	pciiddeviceinfo = make(map[string]string)
	nodepcis, err := c.GetNodePciList(node)
	if err != nil {
		log.Printf("Error listing NodePcis %+v\n", err)
		os.Exit(1)
	}
	pcisinfo, ispci := nodepcis["data"].([]interface{})
	if ispci {
		for pci := range pcisinfo {
			pciinfo := pcisinfo[pci].(map[string]interface{})
			if string(pciinfo["vendor_name"].(string)) == "NVIDIA Corporation" {
				vid := strings.TrimSpace(string(pciinfo["id"].(string)))
				vdevice := strings.TrimSpace(string(pciinfo["device"].(string)))
				isvedioid := strings.HasSuffix(vid, ".0")
				if isvedioid {
					pcislice = append(pcislice, vid)
					pciiddeviceinfo[vid] = vdevice
				}
			}
		}

		return pcislice, pciiddeviceinfo
	}
	return nil, nil
}
...
func (c *Client) GetGxVmConfig(vmr *VmRef) (vmConfig *ConfigQemu, err error) {
	err = c.CheckVmRef(vmr)
	if err != nil {
		return nil, err
	}
	...
	resp, err := c.session.Request("GET", url, nil, nil, &bodyjson)
	if err != nil {
		return nil, err
	}
	...
	vmconfigqemudata := VmConfigQemuData{}
	err = json.Unmarshal(rbody, &vmconfigqemudata)
	if err != nil {
		return &vmconfigqemudata.Data, err
	}
	return nil, nil
