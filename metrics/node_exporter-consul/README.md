#gongxuan add 2022/08/16

在node-exporter原有基础上，添加了consul注册模块。
根据场景修改consulAddress参数默认值，也可以在启动时通过-consul-address参数指定consul地址。
需要额外tag，可以在启动时通过-consul-tags参数指定，默认会把系统类型做为tag打上。
注册说明：
  ID：本机IP+"-node"
